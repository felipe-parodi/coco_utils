"""COCO Refinement GUI Application.

A Tkinter-based GUI for refining COCO dataset annotations including
...
try:
    ...
    def calculate_shrunk_bboxes(data, percentage):
...
def _load_and_organize_coco_json(
    ...
    """Load and organize COCO annotations with GUI-specific preprocessing."""
    ...
    try:
        ...
        messagebox.showerror("Error", f"Error decoding COCO JSON file: {cleaned_coco_path}")
        return None, None, -1
    except OSError as e:
        messagebox.showerror("Error", f"Error opening COCO JSON file: {cleaned_coco_path}\n{e}")
        return None, None, -1
    ...
    for ann in annotations_in_file:
        ...
        ann_copy = ann.copy()
        if "bbox" not in ann_copy:
            ann_copy["bbox"] = []
        if "keypoints" not in ann_copy:
            ann_copy["keypoints"] = []
        if "num_keypoints" not in ann_copy:
            ann_copy["num_keypoints"] = 0
        ...
        if "iscrowd" not in ann_copy:
            ann_copy["iscrowd"] = 0
        if "category_id" not in ann_copy:
            ann_copy["category_id"] = 1  # Default
        ...
        if bbox and len(bbox) == 4 and kpts_flat and len(kpts_flat) % 3 == 0:
            ...
            if w > 0 and h > 0:  # Ensure valid bbox dimensions
                ...
                if modified_kpts:
                    ...
                    ann_copy["num_keypoints"] = sum(
                        1 for j in range(2, len(kpts_flat), 3) if kpts_flat[j] == 2
                    )
    ...
    if skipped_count > 0:
        print(
            f"Warning: Skipped {skipped_count} annotations because their 'image_id' (frame number) was not found in the video."
        )
...
def load_coco_data(
    ...
    try:
        ...
        messagebox.showerror(
            "Error", f"Error opening/reading COCO JSON file: {cleaned_coco_path}\n{e}"
        )
...
def load_video_data(video_path: str, coco_json_path: str) -> Tuple[
    ...
    if frame_count <= 0 or frame_width <= 0 or frame_height <= 0:
        messagebox.showerror(
            "Error",
            f"Video file has invalid properties (frames/width/height): {cleaned_video_path}",
        )
        cap.release()
        return None, None, None, -1, None
...
def load_results_video_data(video_path: str, results_json_path: str) -> Tuple[
    Optional[Dict[int, Dict]],
    Optional[Dict[int, List[Dict]]],
    Optional[List[Dict]],
    int,
    ...
    for i in range(1, frame_count + 1):
        ...
        frame_map[frame_id] = {
            ...
            "original_height": frame_height,
            "id": frame_id,
            "file_name": f"frame_{frame_id:06d}",
        }
        valid_frame_ids.add(frame_id)
    # --- End Video Loading ---
    ...
    try:
        ...
        messagebox.showerror(
            "Error", f"Error opening/reading results JSON file: {cleaned_results_path}\n{e}"
        )
        cap.release()
        return None, None, None, -1, None
            ...
            if (
                ...
                if w_proc > 0 and h_proc > 0:
                    ...
                    for i_proc in range(0, len(kpts_flat_proc), 3):
                        kx_proc, ky_proc, kv_proc = (
                            kpts_flat_proc[i_proc],
                            kpts_flat_proc[i_proc + 1],
                            kpts_flat_proc[i_proc + 2],
                        )
                        if kx_proc == 0 and ky_proc == 0:
                            new_kx_proc = max(
                                x_proc,
                                min(random.uniform(x_proc, x_proc + w_proc), x_proc + w_proc),
                            )
                            new_ky_proc = max(
                                y_proc,
                                min(random.uniform(y_proc, y_proc + h_proc), y_proc + h_proc),
                            )
                    ...
                    if modified_kpts_proc:
                        ...
                        internal_ann["num_keypoints"] = sum(
                            1 for j in range(2, len(kpts_flat_proc), 3) if kpts_flat_proc[j] == 2
                        )
            # --- End Keypoint Preprocessing ---
    ...
    if not annotation_map and processed_count == 0 and len(instance_info_list) > 0:
        messagebox.showwarning(
            "Annotation Warning",
            "Found instance data in JSON, but none matched the frame numbers from the video.",
        )

    # Extract or define categories if possible
...
class COCORefinementGUI:
    ...
    def __init__(
        ...
        loaded_from_results: bool = False,  # Flag for results format
    ):
        ...
        if not self.source_ids:
            messagebox.showerror(
                "Error", f"No valid {'frames' if is_video_mode else 'images'} loaded. Exiting."
            )
            self.root = None
            if self.video_capture:
                self.video_capture.release()  # Release video if init fails
            return
        ...
        if self.categories and self.categories[0].get("keypoints"):
            ...
            print(
                f"DEBUG: Loaded {len(self.keypoint_names)} keypoint names: {self.keypoint_names[:5]}..."
            )
            ...
            if skeleton_raw:
                ...
                try:
                    ...
                    if min_idx == 0 or (max_idx <= num_kpts - 1):
                        self.skeleton_links = [
                            [int(a), int(b)] for a, b in skeleton_raw if len((a, b)) == 2
                        ]
                    else:
                        self.skeleton_links = [
                            [int(a) - 1, int(b) - 1] for a, b in skeleton_raw if len((a, b)) == 2
                        ]
            ...
            try:
                ...
                if inferred_K:
                    ...
                    if inferred_K == 17:
                        ...
                        print(
                            f"Using standard COCO-17 keypoint names: {self.keypoint_names[:5]}..."
                        )
                    else:
        ...
        if self.source_ids:
            ...
            messagebox.showinfo(
                "Info", f"No {'frames' if is_video_mode else 'images'} found to display."
            )
    ...
    def _create_menu_bar(self):
        ...
        edit_menu.add_checkbutton(
            ...
            variable=self._delete_confirm_var,
            command=self._toggle_delete_confirmation,
        )
    ...
    def _create_toolbar(self, parent):
        ...
        self.source_jumper_entry = ttk.Entry(
            jumper_frame, textvariable=self.source_jumper_var, width=10
        )
        ...
        ttk.Label(indicators_frame, textvariable=self.new_box_label, foreground="blue").pack(
            side=tk.LEFT, padx=5
        )
        ...
        if self.is_video_mode:
            ...
            ttk.Label(
                indicators_frame, textvariable=self.track_id_info_label, foreground="green"
    ...
    def _create_sidebar(self, parent):
        ...
        ttk.Label(header_frame, text="Controls", font=("TkDefaultFont", 10, "bold")).pack(
            side=tk.LEFT
        )
        self.collapse_btn = ttk.Button(
            header_frame, text="◀", width=3, command=self._toggle_sidebar
        )
        ...
        ttk.Button(view_section, text="Toggle View (T)", command=self._toggle_view_mode).pack(
            fill=tk.X, pady=2
        )
        ttk.Button(view_section, text="Reset View (R)", command=self._reset_view).pack(
            fill=tk.X, pady=2
        )
        ttk.Button(view_section, text="Toggle Masks (M)", command=self._toggle_masks).pack(
            fill=tk.X, pady=2
        )
        ...
        self.keypoint_scale = tk.Scale(
            kpt_section,
            from_=1,
            to=20,
            orient=tk.HORIZONTAL,
            command=self._on_keypoint_scale_change,
            length=180,
        )
        ...
        ttk.Button(ann_section, text="New Box (N)", command=self._toggle_new_box_mode).pack(
            fill=tk.X, pady=2
        )
        del_text = "Delete Frame (Del)" if self.is_video_mode else "Delete Image (Del)"
        ttk.Button(ann_section, text=del_text, command=self._delete_current_source).pack(
            fill=tk.X, pady=2
        )
        ttk.Button(ann_section, text="Undo Delete (Ctrl+Z)", command=self._undo_last_deletion).pack(
            fill=tk.X, pady=2
        )
    ...
    def _create_status_bar(self, parent):
        ...
        ttk.Button(status_frame, text="Save", command=self._save_temp_progress).pack(
            side=tk.RIGHT, padx=5
        )
        ttk.Button(status_frame, text="Complete", command=self._complete).pack(
            side=tk.RIGHT, padx=5
        )
    ...
    def _toggle_delete_confirmation(self):
        ...
        print(
            f"Delete confirmation {'enabled' if self.delete_confirmation_enabled else 'disabled'}"
        )
    ...
    def _on_keypoint_scale_change(self, value):
        ...
        # Redraw annotations with updated keypoint size
    ...
    def _on_double_click(self, event):
        ...
        for ann in annotations:
            ...
            if (
                x2_canvas - corner_size <= click_x <= x2_canvas
                and y1_canvas <= click_y <= y1_canvas + corner_size
            ):
                # Zoom to this bbox
    ...
    def _zoom_to_bbox(self, bbox, padding_factor=0.2):
        ...
        print(
            f"Zoomed to bbox at ({x:.1f}, {y:.1f}, {w:.1f}, {h:.1f}) with zoom {target_zoom:.2f}x"
        )
        self._update_zoom_label()
    ...
    def _assign_track_id(self, track_id_to_assign: int):
        ...
        if not self.is_video_mode:
            return  # Only active in video mode
        ...
        if not ann:
            print(
                f"Error: Could not find selected annotation {self.selected_ann_id} to assign track ID."
            )
            return
        ...
        if (
            ...
            and old_track_id in [1, 2]
            and new_track_id in [1, 2]
            and old_track_id != new_track_id
        ):
            prompt_message = (
        ...
        if apply_globally:
            ...
            for src_id in self.source_ids:
                ...
                # Find annotations with the old and updated IDs within this frame
                anns_with_old_id = [
                    fa for fa in frame_anns if fa.get("instance_id") == old_track_id
                ]
                anns_with_new_id = [
                    fa for fa in frame_anns if fa.get("instance_id") == new_track_id
                ]
                ...
                if anns_with_old_id:
                    ...
                    # Only swap the updated ones to old if we actually changed some old ones
            ...
            print(
                f"Globally changed {global_change_count_old_to_new} annotations from {old_track_id} -> {new_track_id}."
            )
            print(
                f"Globally changed {global_change_count_new_to_old} annotations from {new_track_id} -> {old_track_id}."
            )
            ...
            # Check if the ID is already taken by another box in the current frame
            ...
            if id_already_taken:
                messagebox.showwarning(
                    "Track ID Conflict",
                    f"Track ID {new_track_id} is already assigned to another box in this frame. Cannot assign duplicate ID.",
                )
                print(
                    f"Assignment cancelled: Track ID {new_track_id} already exists in frame {current_src_id}."
                )
            elif ann.get("instance_id") != new_track_id:
                ann["instance_id"] = new_track_id
                print(
                    f"Assigned Track ID {new_track_id} to annotation {self.selected_ann_id} on frame {current_src_id}"
                )
                self.modifications_made = True
            else:
    ...
    def _find_valid_index(self, direction=1):
        ...
        if (
            num_sources == 1 and self.source_ids[original_idx] not in self.deleted_source_ids
        ):  # Renamed
            return original_idx

        print(
            f"Info: All {'frames' if self.is_video_mode else 'images'} appear to be marked for deletion."
        )
        return -1
    ...
    def _canvas_to_image_coords(self, canvas_x: float, canvas_y: float) -> Tuple[float, float]:
        ...
        if self.current_cv_image is not None:
            ...
            if "h" in self.flip_mode:
                img_x = W - 1 - img_x
            if "v" in self.flip_mode:
                img_y = H - 1 - img_y
    ...
    def _display_source(self):  # Renamed from _display_image
        ...
        if self.flip_mode == "h":
            img_to_display = cv2.flip(self.current_cv_image, 1)
        elif self.flip_mode == "v":
            img_to_display = cv2.flip(self.current_cv_image, 0)
    ...
    def _load_current_source(self):  # Renamed from _load_current_image
        ...
        if self.is_video_mode:
            if self.video_capture and self.video_capture.isOpened():
                ...
                if ret:
                    ...
                    messagebox.showerror(
                        "Error Reading Frame",
                        f"Failed to read frame {current_src_id} (index {target_frame_zero_idx}) from video.",
                    )
                    error_occurred = True
                    self.current_cv_image = None
            else:
            ...
            try:
                ...
                messagebox.showerror("Error Loading Image", f"Error loading {img_path}:\n{e}")
                error_occurred = True
                self.current_cv_image = None
        ...
        if error_occurred:
            ...
            if next_idx == -1 or next_idx == self.current_idx:
                if self.root:
                    self._complete()  # Close if nothing else to show
                return
            else:
        ...
        if self.is_video_mode:
            display_name = f"Frame {current_src_id}"  # Override for video
        canvas_w = self.canvas.winfo_width()
    ...
    def _draw_annotations(self):
        ...
        self.canvas.delete(
            ...
            "status_text",
            "hover_text",
            "track_id_text",  # Clear specific tags + track ID
        )

        if not self.source_ids or self.current_idx < 0 or self.current_idx >= len(self.source_ids):
            return
        current_src_id = self.source_ids[self.current_idx]
        state = self.source_decision_state.get(
            current_src_id, "undecided"
        )  # Use renamed state dict
    ...
    def _draw_comparison_boxes(self, img_id: int):
        ...
        original_bboxes_by_id = {
            ...
            if ann.get("id") is not None and ann.get("bbox")
        }
        shrunk_bboxes_by_id = {
            ann.get("id"): ann.get("bbox")
            for ann in shrunk_anns
            if ann.get("id") is not None and ann.get("bbox")
        }
        ...
        for ann_id, bbox in original_bboxes_by_id.items():
            ...
            self.canvas.create_rectangle(
                ...
                outline="blue",
                width=2,
                tags=("comparison_box", f"ann{ann_id}_orig"),
            )
        ...
        for ann_id, shrunk_bbox in shrunk_bboxes_by_id.items():
            ...
            if shrunk_bbox and (original_bbox is None or shrunk_bbox != original_bbox):
                ...
                self.canvas.create_rectangle(
                    ...
                    outline="red",
                    width=2,
                    tags=("comparison_box", f"ann{ann_id}_shrunk"),
                )
    ...
    def _draw_active_masks(self, annotations: List[Dict]):
        ...
        for ann in annotations:
            ...
            overlay = overlay.resize(
                (int(W * effective_scale), int(H * effective_scale)), Image.Resampling.NEAREST
            )
    ...
    def _draw_active_poses(self, annotations: List[Dict], color: str = "cyan"):
        ...
        for ann in annotations:
            ...
            if not kpts_flat or len(kpts_flat) % 3 != 0:
                continue
            ...
            if self.skeleton_links:
                ...
                for start_idx, end_idx in self.skeleton_links:
                    if start_idx < len(kpts) and end_idx < len(kpts):
                        ...
                        start_kpt_name = (
                            self.keypoint_names[start_idx]
                            if start_idx < len(self.keypoint_names)
                            else f"kpt_{start_idx}"
                        )
                        end_kpt_name = (
                            self.keypoint_names[end_idx]
                            if end_idx < len(self.keypoint_names)
                            else f"kpt_{end_idx}"
                        )
            ...
            print(
                f"DEBUG: Visibility counts - v=0: {v_counts[0]}, v=1: {v_counts[1]}, v=2: {v_counts[2]}"
            )
            ...
            for i, (x, y, v) in enumerate(kpts):
                ...
                print(
                    f"DEBUG: Drawing keypoint {i} ({kpt_name}) at ({x}, {y}) with visibility {v}, color {kpt_color}"
                )
                ...
                self.canvas.create_oval(
                    center_xc - kpt_draw_radius,
                    center_yc - kpt_draw_radius,
                    center_xc + kpt_draw_radius,
                    center_yc + kpt_draw_radius,
                    fill=fill_color,
    ...
    def _on_mouse_down(self, event):
        ...
        self.selected_ann_id = None  # Reset selection on click
        self.drag_type = None
        self.selected_corner = None
        self.selected_kpt_index = None
        ...
        if self.view_mode == "pose":
            items = self.canvas.find_overlapping(
                event.x - self.keypoint_radius,
                event.y - self.keypoint_radius,
                event.x + self.keypoint_radius,
                event.y + self.keypoint_radius,
            )
            for item in reversed(items):
                ...
                if "keypoint" in tags and "editable" in tags:
                    ...
                    if ann_tag and kpt_tag:
                        ...
                        print(
                            f"Selected keypoint {self.selected_kpt_index} for ann {self.selected_ann_id}"
                        )
                        return  # Stop checking

        # 1. Handle Clicks
        items = self.canvas.find_overlapping(
            event.x - self.handle_size / 2,
            event.y - self.handle_size / 2,
            event.x + self.handle_size / 2,
            event.y + self.handle_size / 2,
        )
        for item in reversed(items):
            ...
            if "handle" in tags and "editable" in tags:
                ann_tag = next((t for t in tags if t.startswith("ann")), None)
                corner = next(
                    (t for t in tags if t in ["topleft", "topright", "bottomleft", "bottomright"]),
                    None,
                )
        ...
        for ann in reversed(annotations):  # Check topmost first
            ...
            if min(x1c, x2c) < event.x < max(x1c, x2c) and min(y1c, y2c) < event.y < max(y1c, y2c):
                ...
                print(
                    f"Selected box interior for ann {self.selected_ann_id} (potential move/ID assign)"
                )
                clicked_on_box = True
    ...
    def _on_mouse_drag(self, event):
        ...
        if self.drag_type == "create" and self.new_box_start:
            ...
            # --- Resize Logic ---
                ...
                current_x_img, current_y_img = self._canvas_to_image_coords(
                    current_canvas_x, current_canvas_y
                )
                x_img, y_img, w_img, h_img = ann["bbox"]
                orig_x1_img, orig_y1_img, orig_x2_img, orig_y2_img = (
                    x_img,
                    y_img,
                    x_img + w_img,
                    y_img + h_img,
                )
                final_x1_img, final_y1_img, final_x2_img, final_y2_img = (
                    orig_x1_img,
                    orig_y1_img,
                    orig_x2_img,
                    orig_y2_img,
                ...
                if "left" in self.selected_corner:
                    final_x1_img = current_x_img
                if "right" in self.selected_corner:
                    final_x2_img = current_x_img
                if "top" in self.selected_corner:
                    final_y1_img = current_y_img
                if "bottom" in self.selected_corner:
                    final_y2_img = current_y_img
                ...
                if (
                    ...
                    for i in range(0, len(kpts_flat), 3):
                        ...
                        # Scale relative to the top-left corner
                ...
                if final_x1_img > final_x2_img:
                    ...
                    self.selected_corner = corner_map_h.get(
                        self.selected_corner, self.selected_corner
                    )
                if final_y1_img > final_y2_img:
                    ...
                    self.selected_corner = corner_map_v.get(
                        self.selected_corner, self.selected_corner
                    )
            ...
            # --- Keypoint Drag Logic ---
                ...
                if not ann or "keypoints" not in ann or not ann["keypoints"]:
                    print(
                        f"Error: Could not find annotation {self.selected_ann_id} or keypoints for drag."
                    )
                    self.drag_type = None
                    return
                ...
                if kpt_base_idx + 1 >= len(kpts_flat):
                    print(
                        f"Error: Invalid keypoint index {self.selected_kpt_index} for ann {self.selected_ann_id}."
                    )
                    self.drag_type = None
                    return
    ...
    def _on_right_click(self, event):
                    ...
                    if ann_tag and kpt_tag:
                        ...
                        if ann and ann.get("keypoints"):
                            ...
                            if kpt_base_idx + 2 < len(kpts_flat):
                                ...
                                print(
                                    f"Toggled visibility for keypoint {kpt_idx_to_toggle} ann {ann_id_to_toggle} from {visibility_names.get(current_v, current_v)} to {visibility_names.get(new_v, new_v)}"
                                )
        ...
        if not toggled_keypoint:
            ...
            items = self.canvas.find_overlapping(
                event.x - self.handle_size,
                event.y - self.handle_size,
                event.x + self.handle_size,
                event.y + self.handle_size,
            )
            ...
            for item in reversed(items):
                tags = self.canvas.gettags(item)
                if (
                    "handle" in tags or "keypoint" in tags
                ) and "editable" in tags:  # Also check kpts
                    ann_tag = next((t for t in tags if t.startswith("ann")), None)
                    if ann_tag:
            ...
            if ann_id_to_delete is None:
                ...
                for ann in reversed(annotations):
                    ...
                    x1c, y1c = self._image_to_canvas_coords(x_img, y_img)
                    x2c, y2c = self._image_to_canvas_coords(x_img + w_img, y_img + h_img)
                    if min(x1c, x2c) < event.x < max(x1c, x2c) and min(y1c, y2c) < event.y < max(
                        y1c, y2c
                    ):
    ...
    def _toggle_view_mode(self, event=None):
        ...
        if state != "undecided":
            ...
            messagebox.showinfo(
                "Info", "Please choose Original ('A') or Shrunk ('D') before toggling view."
            )
    ...
    def _toggle_flip_mode(self, event=None):
        ...
        modes = ["none", "h", "v", "hv"]
        try:
            current_mode_idx = modes.index(self.flip_mode)
        except ValueError:
            current_mode_idx = 0
    ...
    def _toggle_new_box_mode(self, event=None):
        ...
        if state == "undecided" and not self.creating_new_box:
            messagebox.showinfo(
                "Info", "Please choose Original ('A') or Shrunk ('D') before creating new boxes."
            )
            return
        ...
        try:
            ...
            if target_num in self.source_ids:  # Check if the ID actually exists
                ...
                if target_idx != self.current_idx:
                    print(
                        f"Jumping to {'frame' if self.is_video_mode else 'image'} {target_num} (index {target_idx})..."
                    )
                ...
                messagebox.showwarning(
                    "Invalid ID",
                    f"Please enter a valid {'frame' if self.is_video_mode else 'image'} number between {self.source_ids[0]} and {self.source_ids[-1]}.",
                )
                self._update_jumper_text()
        except (ValueError, IndexError):
            messagebox.showwarning(
                "Invalid Input",
                f"Please enter the {'frame' if self.is_video_mode else 'image'} number (e.g., '100').",
            )
            self._update_jumper_text()
        except Exception as e:
    ...
    def _update_jumper_text(self):
        ...
        total_sources = len(self.source_ids)
        current_display_num = (
            self.source_ids[self.current_idx] if total_sources > 0 and self.current_idx >= 0 else 0
        )
        last_source_num = (
            self.source_ids[-1] if total_sources > 0 else 0
        )  # Use last ID for total display
    ...
    def _delete_current_source(self):  # Renamed from _delete_current_image
        ...
        display_name = src_info.get("file_name", f"ID {current_src_id}")
        if self.is_video_mode:
            display_name = f"Frame {current_src_id}"
        ...
        if self.modifications_made:
            ...
            if response is True:  # Yes
                ...
                if hasattr(self, "save_successful") and self.save_successful:
                    if self.video_capture:
                        self.video_capture.release()  # Release video
                    if self.root:
                        self.root.destroy()
    ...
    def _complete(self):
        ...
        if should_close:
            if self.video_capture:
                self.video_capture.release()  # Release video before closing
            if self.root:
                self.root.destroy()
    ...
    def _save_refined_annotations(self):
        ...
        if self.loaded_from_results:
            ...
            for frame_id in final_source_ids:
                ...
                for internal_ann in annotations_for_frame:
                    ...
                    for i in range(0, len(coco_kpts_flat), 3):
                        ...
                        if "_original_results_instance" in internal_ann:
                            orig_kpts_list = internal_ann["_original_results_instance"].get(
                                "keypoints", []
                            )
                            kpt_idx = i // 3
                            if kpt_idx < len(orig_kpts_list):
                                orig_kpts_list[kpt_idx] = [coco_kpts_flat[i], coco_kpts_flat[i + 1]]
                            # Scores are not updated here, kept original
                    ...
                    if "_original_results_instance" in internal_ann:
                        ...
                        print(
                            f"Warning: Reconstructing results instance for ann {internal_ann['id']} - may lose original scores."
                        )
                        results_kpts = [
                            [coco_kpts_flat[i], coco_kpts_flat[i + 1]]
                            for i in range(0, len(coco_kpts_flat), 3)
                        ]
                        x, y, w, h = internal_ann.get("bbox", [0, 0, 0, 0])
                ...
                if frame_instances:  # Only add frame entry if it has instances
                    output_data["instance_info"].append(
                        {"frame_id": frame_id, "instances": frame_instances}
                    )

            print(
                f"Saving {processed_instances_count} instances across {len(output_data['instance_info'])} frames."
            )
            ...
            if not self.is_video_mode:
                ...
                for src_id in self.source_ids:
                    if (
                        src_id not in self.deleted_source_ids
                        and self.source_decision_state.get(src_id) == "undecided"
                    ):
                ...
                if undecided_sources:
                    ...
                    if not messagebox.askyesno("Undecided Images", msg):
                        ...
                        print(
                            f"Proceeding to save. {num_undecided} undecided image(s) will use their ORIGINAL annotations."
                        )
                        for src_id_undecided in undecided_sources:
                            ...
                            if (
                                current_modified_anns is None or not current_modified_anns
                            ):  # If key doesn't exist or list is empty
                                ...
                                self.source_decision_state[src_id_undecided] = (
                                    "original_defaulted_on_save"
                                )
                                print(
                                    f"Image ID {src_id_undecided}: Defaulted to original annotations."
                                )
                                # If we are defaulting, it implies a modification might be needed for saving state.
                                self.modifications_made = (
                                    True  # Ensure this is True if we are defaulting
                                )
            ...
            for src_id in final_source_ids:
                ...
                for ann in annotations_to_save:
                    ...
                    if "num_keypoints" not in ann_copy:
                        ann_copy["num_keypoints"] = sum(
                            1
                            for i in range(2, len(ann_copy.get("keypoints", [])), 3)
                            if ann_copy.get("keypoints", [])[i] > 0
                        )
                    if "area" not in ann_copy and ann_copy["bbox"] and len(ann_copy["bbox"]) == 4:
                        ann_copy["area"] = ann_copy["bbox"][2] * ann_copy["bbox"][3]
                    elif "area" not in ann_copy:
            ...
            if self.is_video_mode and missing_track_id_count > 0:
                print(
                    f"Warning: {missing_track_id_count} annotations were saved with a default track ID (0) because none was assigned."
                )
    ...
    def _accept_shrunk(self, event=None):
        ...
        if self.source_decision_state.get(current_src_id) == "undecided":
            if current_src_id in self.shrunk_annotations:
                ...
                messagebox.showwarning(
                    "Shrink Data Missing",
                    f"No shrunk annotation data available for image {current_src_id}.",
                )
        else:
            print(f"Decision already made for image {current_src_id}.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Interactively refine COCO bounding boxes and poses, with optional video mode."
    )

    # Input Source Arguments (mutually exclusive groups might be better, but simple check works)
    parser.add_argument(
        "--coco_path",
        help="Path to the COCO annotation JSON file (used for image mode OR video mode).",
    )
    parser.add_argument(
        "--results_file", help="Path to the custom results JSON file (used ONLY for video mode)."
    )
    parser.add_argument(
        "--img_dir", help="Path to the directory containing images (REQUIRED for image mode)."
    )
    ...
    parser.add_argument(
        ...
        default="bbox",
        choices=["bbox", "pose"],
        help="Initial view mode ('bbox' or 'pose').",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="Optional percentage (0-100) to shrink boxes in IMAGE mode. If provided, starts in comparison mode.",
    )
    ...
    if is_video_mode:
        if not args.coco_path and not args.results_file:
            print(
                "Error: Video mode requires either --coco_path OR --results_file for annotations."
            )
            exit(1)
    ...
    if is_video_mode:
        ...
        print(
            f"--- Running in VIDEO mode {'(Results Format)' if loaded_from_results else '(COCO Format)'} ---"
        )
        print(f"Video Path: {video_path}")
        ...
        if loaded_from_results:
            # Load using the results parser
            source_data, anno_data, category_data, next_ann_id, video_capture = (
                load_results_video_data(video_path, input_json_path)
            )
        else:
            ...
            if perform_shrink_comparison:
                ...
                temp_coco_data_for_shrinking = {
                    "images": [
                        ...
                        for img_id, info in source_data.items()
                    ],
                ...
                try:
                    ...
                    for ann in shrunk_full_coco_data.get("annotations", []):
                        img_id = ann["image_id"]
                        if img_id in source_data:
                            shrunk_anno_data.setdefault(img_id, []).append(ann)
                    print("Shrunk annotation calculation complete.")
                except ImportError:
                    print(
                        "Warning: Cannot calculate shrunk annotations - coco_labels_utils import failed. Proceeding without comparison."
                    )
                    perform_shrink_comparison = False
                    shrunk_anno_data = {}
                except Exception as e:
                    try:
                        tk.Tk().withdraw()
                        messagebox.showerror(
                            "Shrinking Error", f"Failed to calculate shrunk boxes: {e}"
                        )
                    except tk.TclError:
                        print(f"Error: Tkinter not available for error dialog.")
