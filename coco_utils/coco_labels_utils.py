...
def shrink_bbox(
    ...
    if not (0 < shrink_percent < 100):
        logger.warning(
            f"Invalid shrink_percent ({shrink_percent}). Returning original bbox."
        )
        return bbox
...
def shrink_coco_bboxes(
    ...
    if interactive:
        ...
        if not image_dir_path.is_dir():
            raise FileNotFoundError(
                f"Image directory not found for interactive mode: {image_dir}"
            )
    ...
    if interactive:
        ...
        if not image_filename:
            raise ValueError(
                f"'file_name' missing for example image ID {first_modified_id}."
            )
        ...
        try:
            ...
            raise FileNotFoundError(
                f"Image file not found at '{image_path}' for visualization."
            )
        except Exception as e:
            raise VisualizationError(
                f"Error loading image '{image_path}' for visualization: {e}"
            )
        ...
        logger.info(
            f"Displayed: {num_orig_drawn} original boxes, "
            f"{num_shrunk_drawn} shrunk boxes."
        )
